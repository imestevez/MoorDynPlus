...
/// \file MoorDyn.h \brief Defines the class \ref MoorDyn.
...
private:
  ...
  void AllocateMemoryIntegration(const unsigned size); /// Allocates memory for arrays of integration
...
public:
  ...
  int LinesInit(double X[],double XD[],const std::string filexml
    ...
    ,const unsigned numFts=0); ///Funtion to Init the system when there is a coupling with externan software
