...
void Line::ReadXml(JXml *sxml, const std::string &place_in, TiXmlElement* eleLine,const unsigned numLine, std::vector<Connection *> connects, LineDefault * lineDef) {
	...
	if (eleLine) { 
		...
		std::string output_str=sxml->GetAttributeStr(eleLine, "output", true, "yes");
		std::transform(output_str.begin(), output_str.end(), output_str.begin(), ::tolower);
		std::size_t yes=output_str.find("yes");
		std::size_t no=output_str.find("no");
		OutputLine=(yes!=std::string::npos ? true : no!=std::string::npos ? false : true);
		...
		if (NConnections!= 2) { Run_Exceptioon("Error: Just 2 connections are allowed. Found: " + ToString(NConnections) + "."); }
...
void Line::ConfigLine(vector<Connection*> connections){
	...
	else{Run_Exceptioon("Error: The First Connection of the Line " + ToString(Number) + " must be fixed of vessel.");}
...
Connection * Line::SearchConnect(const unsigned ref, std::vector<Connection *> connects){
	...
	Run_Exceptioon("Error: There aren't connconnection with ref: " + ToString(ref) + " in linedefault block." );
	return NULL	;
}
//==============================================================================
/// Check if the output flag is in the list of posibilities.
...
void Line::CheckInputParameters() {
	...
	if (UnstrLen<distance) { 
		string tex="Error: The length of the line " + ToString(Number) 
			+ " must be at least of " + ToString(distance) + "m.";
		Run_Exceptioon(tex); 
	}
	return;
}
...
void Line::Initialize(double* X) {
	...
	if (-(depth)>R[0][2]) { 
		string tex="Error: water depth is shallower than the anchor heigth in Line " + ToString(Number)+".";
		Run_Exceptioon(tex); 
	}
...
//==============================================================================
tdouble3 Line::GetPositionOutput(OutProperties * outP) {
	if (outP->GetTypeCon()==TypeConnection::fixed) {
		AnchConnect=GetAnchConnect();
		return	AnchConnect->GetPositions();
	}
	else if (outP->GetTypeCon()==TypeConnection::vessel) {
		FairConnect=GetFairConnect();
		return	FairConnect->GetPositions();
	}
	else { return TDouble3(0); }
}
//==============================================================================
/// Returns the velocity (X,Y,Z)
//==============================================================================
tdouble3 Line::GetVelocityOutput(OutProperties * outP) {
	return TDouble3(Rd[outP->GetNodeID()][0], Rd[outP->GetNodeID()][1], Rd[outP->GetNodeID()][2]);
}
//==============================================================================
/// Returns the force (X,Y,Z)
//==============================================================================
tdouble3 Line::GetForceOutput(OutProperties * outP) {
	return TDouble3(Fnet[outP->GetNodeID()][0], Fnet[outP->GetNodeID()][1], Fnet[outP->GetNodeID()][2]);
}
//==============================================================================
/// Returns the Tension
//==============================================================================
double Line::GetTensionOutput(OutProperties * outP) {
	return GetNodeTen(outP->GetNodeID());
}
//==============================================================================
...
void Line::DoRHS(const double* X, double* Xd, const double time, const double dt) {
		...
		for (int J=0; J<3; J++) {
			...
			if (std::isnan(ldstr_top)) { 
				string tex="Error:  NaN value detected in MoorDyn state at dynamic relaxation time "
					+ ToString(time) + " s. Try to reduce the EA or massDenInAir or increase the segments in line " + ToString(Number) + ".";
				Run_Exceptioon(tex); 
			}

		}
		if (Lstr[i]==double(0)) { 
			string tex="Error: Lstr[" + ToString(i) + "]==" 				
				+ ToString(Lstr[i]) + ". lstr_squared= " + ToString(lstr_squared) 
				+ " sqrt= " + ToString(sqrt(lstr_squared)) + " Cannot divide by 0. Check line Number " 
				+ ToString(Number) + " of body " + ToString(*BodyId) + ".";
			Run_Exceptioon(tex); 
		}
	...
	if (WaveKin==0) {   // if Wave Kinematics haven't been calculated.   ...this is a local Line switch (so wave kinematics can be enabled/disabled for individual lines)
	   ...
	   // get interpolation constant and wave time step index
		...
		if (wordy>2) {if (Number==1) { printf("\n t=%.f, U[4][0]= %.f\n",t, U[4][0]); }	}
	}

	CalculateMassMatrix(); //Calls calculate Maºss matrix
		...
		for (int I=0; I<3; I++) {
			...
			for (int J=0; J<3; J++) {
				...
				if (std::isnan(Fnet[i][J])) {
					string tex="Error: Fnet[" + ToString(i) + "][" + ToString(J) 
...
void Line::CalculateMassMatrix() {
	...
	for (unsigned i=0; i<=N; i++) {
		...
		for (int I=0; I<3; I++) {
			for (int J=0; J<3; J++) {M[i][I][J]=m_i*eye(I, J) + Env->GetRho_w()*v_i *(Can*(eye(I, J) - Q[i][I] * Q[i][J]) + Cat*Q[i][I] * Q[i][J]);		}
		}
