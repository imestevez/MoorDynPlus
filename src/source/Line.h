...
private:
	...
	doubleC WGNC_Fact; 		 	///< This factor is needed by the discrete time inverse Fourier transform to ensure that the time series WGN process has unit variance // sqrt( pi/(dw*WaveDT) );   
	...
	void CheckChannels(JXml *sxml, TiXmlElement * elel);	/// Checks if the output flag is in the list of posibilities
	void ConfigLine(vector<Connection*> connections); 		///Configures the start and end Connection
...
public:
	...
	double GetTensionOutput(OutProperties * outP);					/// Returns the tension to write in output file
	tdouble3 GetPositionOutput(OutProperties * outP);				/// Returns the force to write it in output file
	tdouble3 GetVelocityOutput(OutProperties * outP);				/// Returns the force for write it in output file
	tdouble3 GetForceOutput(OutProperties * outP);					/// Returns the force for write it in output file
	...
	void GetAnchStuff(vector<double> &Fnet_out, vector< vector<double> > &M_out);/// Writes Forces and Mass into the vectors, which are pased how parameters. For Anchors
	void GetFairStuff(vector<double> &Fnet_out, vector< vector<double> > &M_out);/// Writes Forces and Mass into the vectors, which are pased how parameters. For Fairleads
	void BreakLine();
	void CheckTension();
#ifdef USEGL	
	void drawGL(void);
