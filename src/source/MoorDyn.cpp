...
/// \file MoorDyn.cpp \brief Implements the class \ref MoorDyn.
...
//==============================================================================
MoorDyn::~MoorDyn() {
  funmd::FreeVector(Lines);
  funmd::FreeVector(Connections); 
  funmd::FreeVector(Bodies); 
  funmd::FreeArray2D(Ffair,NFairs);
  funmd::FreeArray2D(rFairi,NFairs);
  funmd::FreeArray2D(rdFairi,NFairs);
  Reset();
}
...
void MoorDyn::Reset() {
  ...
  if(FairArrays) {
    if(FairleadPos)   funmd::FreeArray3D(FairleadPos,NBodies);    FairleadPos=NULL;
    if(FairleadVel)   funmd::FreeArray3D(FairleadVel,NBodies);    FairleadVel=NULL;
    if(FairleadForce) funmd::FreeArray3D(FairleadForce,NBodies);  FairleadForce=NULL;
  }
  XmlReaded=FairArrays=HeaderProg=false;
  NBodies=NLines=NConnections=0;
  NFairs =NAnchs=NConns=0;
  TimeMax=DtOut=0;
...
void MoorDyn::ReadXml(JXml *sxml) {
  ...
  TiXmlElement *rootele=root->ToElement();
  sxml->CheckElementNames(rootele,true,"solverOptions bodies connects lines savedata output");
  ...
  if(sxml->GetNode(place,false)) {
    unsigned nBodiesTag=sxml->CountElements(root,"bodies");
  ...
  if(sxml->GetNode(place,false)) {  
    ...
    unsigned nConnectsTag=sxml->CountElements(root,"connects");
  ...
  if(!sxml->GetNode(place,false)) {Run_Exceptioon("Cannot find the element \'"+place+"\'.");  }
    ...
    unsigned nLinesTag=sxml->CountElements(root,"lines");
...
void MoorDyn::CheckBodyRefs(){
  ...
  for(unsigned c=0; c<fairs.size(); c++){
    ...
    if(!GetBody(bRef)){ Run_Exceptioon("Error: Cannot find someone Body with id "+funmd::ToString<unsigned>(bRef)+"."); }
  }
  return;
}
//==================================================================================
...
void MoorDyn::CheckFtIds(const unsigned ids[],const unsigned numFt) {
  ...
  for(unsigned i=0; i<numFt; i++) {
    ...
    if(!body) { Run_Exceptioon("Error: Cannot find someone Body with id "+funmd::ToString<unsigned>(ids[i])+"."); }
  }
  return;
}
//==============================================================================
...
void MoorDyn::ConfigConnections(std::vector<Connection*> connects){
  ...
  for(unsigned l=0; l<NLines; l++){
    ...
    for(unsigned c=0; c<line->GetNConnections(); c++){
      ...
      if(connection->GetType()!=2)connections_temp.push_back(connection);//If the connection isn't a connect
  ...
  for(unsigned m=0; m<Bodies.size(); m++) {
    if(Bodies[m]) {  //Check if isnt't NULL
      if(Bodies[m]->GetRef()==ref) {
        string tex="Error: The ref="+funmd::ToString(ref) +" of the bodies is duplicated.";
        Run_Exceptioon(tex);
      }
    }
  }
...
void MoorDyn::CheckConnReferences(const unsigned ref,std::vector<Connection*> conns) {
  ...
  for(unsigned m=0; m<conns.size(); m++) {
    if(conns[m]->GetRef()==ref) {
      string tex="Error: The conref="+funmd::ToString(ref)+" of the connects is duplicated.";
      Run_Exceptioon(tex);
    }
  }
}
...
void MoorDyn::Setup() {
  ...
  //PRINT:
  if(Debug) { printf("\n    ---------- START Setup --------\n\n"); }
  double *Ucurrent=funmd::GetArray(3);//TODO: checks the meanig of this variable
  ...
  for(unsigned l=0; l<NLines; l++) {
    ...
    for(unsigned c=0; c<line->GetNConnections(); c++) {
      ...
      if((connection->GetType()<0)||(connection->GetType()>2)){Run_Exceptioon("Error: Wrong Connection type found: "+ funmd::ToString(connection->GetType())+" in Line: "+funmd::ToString(line->GetNumber())+ ".");}
...
void MoorDyn::PrepareState() {
  ...
  Ffair  =funmd::GetArray2D(NFairs,3);
  rFairi =funmd::GetArray2D(NFairs,3);
  rdFairi=funmd::GetArray2D(NFairs,3);
}
...
void MoorDyn::StoreConnPositions() {
  ...
  rFairtS=funmd::GetArray2D(nConn_in,3); //returns a double **
...
void MoorDyn::DoDynamicRelaxIC() {
  ...
  double *Tensions=funmd::GetArray(NFairs * 3 * niic);  // pointer to double for store tensions for analyzing convergence
  double *FairTens=funmd::GetArray(NFairs); // pointer to double for store tensions for analyzing convergence
  double *FairTensLast=funmd::GetArray(NFairs); // pointer to double for store tensions for analyzing convergence
  double *FairTensLast2=funmd::GetArray(NFairs); // pointer to double for store tensions for analyzing convergence
  ...
  for(iic=0; iic<niic; iic++) {
    ...
    IApi::ShowProgressBar(progress);    // Show progres by console
...
//==================================================================================
int MoorDyn::LinesInit(double X[],double XD[],const std::string filexml
...
int MoorDyn::FairleadsCalc(const unsigned numFts,double*** rFairIn,double*** rdFairIn,double*** fFairIn,double* t_in,double *dt_in) {
  ...
  if(dtC>0) { // if DT>0, do simulation, otherwise leave passed fFairs unadjusted.
    ...
    for(unsigned b=0; b<numFts; b++){
      if(CheckBody(b)){
        string tex="There isn't a mooring with id "+funmd::ToString(b)+".";
        Run_Exceptioon(tex);
      }
      Body *body=Bodies[b];
    ...
    if(NdtM<1) {
      string tex="Error: dtC is less than dtM. ("
       +funmd::ToString(dtC)+"<"+funmd::ToString(Env->GetDtM0())+").";
      Run_Exceptioon(tex);
    }
...
bool MoorDyn::CheckLine(const unsigned ftid,const unsigned numLine) {
  ...
  if(CheckBody(ftid)){
    string tex="There isn't a body with id "+funmd::ToString(ftid)+".";
    Run_Exceptioon(tex);
  }
...
bool MoorDyn::CheckLine(const unsigned numLine) {
  ...
  if((numLine<0) || (numLine>=NLines)) {
    string tex="Error: The range of Lines is ["
     +funmd::ToString(0)+","+funmd::ToString(NLines-1)
     +"]. Sent: "+funmd::ToString(numLine);
    Run_Exceptioon(tex);
  }
  Line *line=Lines[numLine]; 
  //-If the line doesn't exist
...
unsigned MoorDyn::GetSegsCount(const unsigned numLine) {
  ...
  if(CheckLine(numLine)){
    string tex="There isn't a line with number "+funmd::ToString(numLine)+".";
    Run_Exceptioon(tex);
  }
  unsigned numSeg=0;
  Line *line=Lines[numLine]; 
...
unsigned MoorDyn::GetSegsCount(const unsigned ftid,const unsigned numLine) {
  ...
  if(CheckLine(ftid,numLine)){
    string tex="There isn't a line with number "+funmd::ToString(numLine)+" for the Body "+funmd::ToString(ftid)+".";
    Run_Exceptioon(tex);
  }
  unsigned numSeg=0;
  Body *body=Bodies[ftid];   
...
int MoorDyn::GetNodePos(const unsigned numLine,const int NodeNum,double pos[3]) {
  ...
  if(nodePos<0) { //if is -1, an error was produced
    string tex="Error: The range of the Nodes is ["
     +funmd::ToString(0)+","+funmd::ToString(line->GetN())
     +"]. Sent: "+funmd::ToString(NodeNum)
     +" of Line "+funmd::ToString(numLine)+".";
    Run_Exceptioon(tex);
  }
  return 0;
}
