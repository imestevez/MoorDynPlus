...
set name=CaseMoorings3D_RealScale
...
%dualsphysicsgpu% -gpu %dirout%/%name% %dirout% -stable
if not "%ERRORLEVEL%" == "0" goto fail

:postprocessing
set dirout2=%dirout%\floatinginfo
%floatinginfo% -dirdata %diroutdata% -onlymk:60 -savemotion -savedata %dirout2%/FloatingMotion 
if not "%ERRORLEVEL%" == "0" goto fail

set dirout2=%dirout%\particles
%partvtk% -dirdata %diroutdata% -savevtk %dirout2%/PartFluid -onlytype:-all,+fluid 
if not "%ERRORLEVEL%" == "0" goto fail
%partvtk% -dirdata %diroutdata% -savevtk %dirout2%/PartMoving -onlytype:-all,+moving 
if not "%ERRORLEVEL%" == "0" goto fail
...
%isosurface% -dirdata %diroutdata% -saveiso %dirout2%/Surface 
if not "%ERRORLEVEL%" == "0" goto fail
