...
export name=CaseConnectedBoxes
...
${partvtk} -dirdata ${diroutdata} -savevtk ${dirout2}/PartPiston -onlytype:-all,+moving 
if [ $? -ne 0 ] ; then fail; fi
...
${floatinginfo} -dirdata ${diroutdata} -savemotion -onlymk:55,60 -savedata ${dirout2}/Vessel
if [ $? -ne 0 ] ; then fail; fi
